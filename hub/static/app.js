...
(() => {
  ...
  // One polished tooltip box (styled as .sc-tooltip in app.css) driven by a
  // DELEGATED listener: any control with a native `title` (or `data-tip`) shows
  // the box on hover/focus, with the docs "?" INSIDE the box. We read the
  // control's existing `title`, adopt it into `data-tip`, and strip the native
  // `title` so the browser's small unstyled box never appears. Every control that
  // had a title keeps its tooltip — no per-call-site migration needed. The "?"
  // deep-links to the help section for the current view (or the element's
  // `data-tip-help` when set). Text is set via textContent, never innerHTML.
  function tip(text, anchor) { const a = {}; if (text) a["data-tip"] = text; if (anchor) a["data-tip-help"] = anchor; return a; }
  function withTip(node, text, anchor) { if (node && text) node.setAttribute("data-tip", text); if (node && anchor) node.setAttribute("data-tip-help", anchor); return node; }
  const HELP_VIEW_ANCHOR = { nodes: "nodes", macros: "macros", events: "events", settings: "settings", runbooks: "runbooks" };
  function viewHelpAnchor() { return (state && HELP_VIEW_ANCHOR[state.view]) || "overview"; }
  const tipEl = {};
  let tipHideTimer = null, tipTarget = null;
  ...
  function ensureTooltip() {
    ...
    const helpA = h("a", { class: "sc-tip-help", target: "_blank", rel: "noopener",
      "aria-label": "Open the docs for this feature (new tab)", onClick: (e) => e.stopPropagation() }, "?");
    const foot = h("div", { class: "sc-tip-foot" }, h("span", { class: "sc-tip-learn" }, "Learn more"), helpA);
    const body = h("div", { class: "sc-tip-text" });
    const box = h("div", { class: "sc-tooltip", role: "tooltip", onMouseEnter: clearTipHide, onMouseLeave: scheduleTipHide }, body, foot);
  ...
  function positionTooltip(trigger, box) {
    ...
    if (top + bh > vh - m && r.top - gap - bh > m) top = r.top - gap - bh;
  ...
  function showTooltip(trigger) {
    let text = trigger.getAttribute("data-tip");
    if (!text) {
      text = trigger.getAttribute("title");
      if (text) { trigger.setAttribute("data-tip", text); trigger.removeAttribute("title"); }  // adopt + suppress native
    }
    if (!text) return;
    clearTipHide();
    tipTarget = trigger;
  ...
  function hideTooltip() {
    clearTipHide(); tipTarget = null;
    if (tipEl.box) { tipEl.box.classList.remove("sc-tip-on"); tipEl.box.style.display = "none"; }
  }
  function installTooltips() {
    const trig = (e) => (e.target && e.target.closest) ? e.target.closest("[title],[data-tip]") : null;
    document.addEventListener("mouseover", (e) => { const t = trig(e); if (!t) return; if (t === tipTarget) { clearTipHide(); return; } showTooltip(t); });
    document.addEventListener("mouseout", (e) => { const t = trig(e); if (!t) return; const to = e.relatedTarget; if (to && tipEl.box && (to === tipEl.box || tipEl.box.contains(to))) return; if (to && t.contains(to)) return; scheduleTipHide(); });
  ...
  // The standalone "?" icon is retired: the docs "?" now lives INSIDE the custom
  // tooltip box (see showTooltip). Returning null removes the separate icons at
  // every call site (h() skips null children) with no other edits. Section
  // headers that called helpLink(anchor) can pass that anchor to the box by
  // carrying a `data-tip-help`; otherwise the box "?" uses the current view's
  // help section, which is the right doc for the control anyway.
  function helpLink() { return null; }
  ...
  async function boot() {
    ...
    renderShell();
    installTooltips();   // delegated listener drives the custom tooltip singleton
