...
graph TB
  ...
  OP["Operator browser"] -->|"HTTP + WebSocket<br/>(via VPN / tunnel)"| HUB
```
...
**Hardware (per node):** Raspberry Pi Pico (RP2040), WIZnet W5100S Ethernet HAT,
a USB data cable to the target, an Ethernet drop. **Hub:** one Raspberry Pi
Zero 2 W with a static IP. **Fabric:** a managed switch / isolated VLAN.
