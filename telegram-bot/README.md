...
```bash
# 1. On the hub host (or any box that can reach the hub's :8080)
bash telegram-bot/scripts/install.sh          # own venv + .env from template

# 2. Create a bot with @BotFather, then edit telegram-bot/.env:
#    TELEGRAM_BOT_TOKEN         from BotFather
#    TELEGRAM_ALLOWED_CHAT_IDS  your numeric chat id (message the bot, read the log)
#    SHELL_TOTP_SECRET          python -c "import pyotp;print(pyotp.random_base32())"
#      → add that secret to your authenticator app as a manual key

# 3. Run it
bash telegram-bot/scripts/run.sh               # foreground (dev)
# …or as a service:
bash telegram-bot/scripts/install-service.sh   # systemd unit
```
...
```bash
telegram-bot/.venv/bin/python -m pytest telegram-bot/tests        # unit tests
telegram-bot/.venv/bin/python telegram-bot/tests/smoke.py         # import + wiring smoke
```
